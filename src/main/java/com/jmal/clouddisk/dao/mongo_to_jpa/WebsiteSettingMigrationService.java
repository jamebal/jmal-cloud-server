...
public class WebsiteSettingMigrationService {

    private final MongoTemplate mongoTemplate;

    private final WebsiteSettingRepository jpaRepository;
    ...
    public MigrationResult migrateWebsiteSetting() {
        ...
        log.info("开始迁移 WebsiteSetting 数据从 MongoDB 到 JPA");
        ...
        try {
            while (true) {
                ...
                List<WebsiteSettingDO> mongoDataList = mongoTemplate.find(query, WebsiteSettingDO.class);
                ...
                try {
                    ...
                    // 如果批量保存失败，尝试逐条保存
                    for (WebsiteSettingDO mongoData : mongoDataList) {
                        try {
                            jpaRepository.save(mongoData);
                            result.addSuccess(1);
                            result.incrementProcessed();
                        } catch (Exception ex) {
                            log.error("保存 WebsiteSetting 数据失败, ID: {}, 错误: {}", mongoData.getId(), ex.getMessage());
